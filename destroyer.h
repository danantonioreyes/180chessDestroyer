...
void expansion(VERTEX* arg, int turn) {
	...
	for(i=0; i<5000; i++) arg->children[i] = NULL;
	...
	for(j=0; j<=7; j++) {
		...
		for(y=0; y<=7; y++) {
			if (valid_move(arg->boardstate,i,j,x,y,turn)) {
				...
				printf("Generated child depth %d\n", arg->children[c]->depth);
				...
				c++; //increment for the next child
			}
		}
	}
