...
int attacked(int **board,int king,int i1,int j1) { //check if a position is attacked,returns from what direction
    int i,upleft,up,upright,right,downright,down,downleft,left;
    
    //king here determines the attacker side
    
    if (safeboard(board,i1,j1)!=BLANK) { //if king is at position i1,j1;
...
int checked(int **board,int king) { //king is checked?
    ...
    return attacked(board,(king+7)%14,i1,j1);
...
int moveok(int **board,int i1,int j1,int i2,int j2) { //if move configuration is ok
    ...
    switch (board[i1][j1]) { //check specific moves
        ...
        case WHITEROOK:
            ...
            break;
        case BLACKPAWN:
            if ((i2==i1+1 AND j2==j1)                                      OR
                (i1==1    AND i2==i1+2 AND j2==j1 AND board[2][j1]==BLANK) OR
        ...
        case WHITEPAWN:
            if ((i2==i1-1 AND j2==j1)                                      OR
                (i1==6    AND i2==i1-2 AND j2==j1 AND board[5][j1]==BLANK) OR
                (i2==i1-1 AND j2==j1-1 AND board[i2][j2]<BLANK)            OR
                (i2==i1-1 AND j2==j1+1 AND board[i2][j2]<BLANK)                 
                ) return 1;
        default:
